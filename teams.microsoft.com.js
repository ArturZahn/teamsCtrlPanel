/* aperte Alt + Z (no vs code) pra esconder essa bagunca ↓↓↓ (isso é o Jquery) */
...
window.onload = () => {
    ...
    var timerWaitLoadHtml = setInterval(()=>{
        if($("#painel").length != 0)
        {
            clearInterval(timerWaitLoadHtml);
            iniciarCodigo();
        }
    }, 100);
}
...
{
    $("#painelheader > div").on("mousedown", ()=>{md(true)});
    $("#painelheader > div").on("mouseup", ()=>{md(false)});

    dragElement(document.getElementById("painel"), document.getElementById("painelheader"));
}

var willClick = false;
function md(mDwn)
{
    if(mDwn) willClick = true;
    else
    {
        if(willClick)
        {
...
function minimize()
{
    ...
    if(!windowState) windowSize = $("#painel")[0].clientWidth;
    tt = parseInt($("#painel")[0].style.left);
    
    if(windowState) $("#painel").removeClass("minimizedWdw");
    else $("#painel").addClass("minimizedWdw");
}
...
function dragElement(elmnt, elmnt2) {
    ...
    function elementDrag(e) {
        ...
        var ppp2 = elmnt.offsetLeft - pos1;
        var ppp1 = elmnt.offsetTop - pos2;
        
        if(ppp2 < 0) ppp2 = 0;
        if(ppp1 < 0) ppp1 = 0;
        
        elmnt.style.left = (ppp2) + "px";
        elmnt.style.top = (ppp1) + "px";
    }
